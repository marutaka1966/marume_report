"""Completed TSE regular-session closes. No live, pre, or post prices."""
...
from v2.jp_session import TOKYO, last_completed_session_date, now_tokyo
...
def yahoo_symbol(code: str) -> str:
    """Holdings stores TSE codes without a suffix. Yahoo uses .T."""
    if "." in code:
        return code
    return f"{code}.T"


def yahoo_source(code: str) -> str:
    return YAHOO_CHART.format(symbol=yahoo_symbol(code))


def fetch_daily_bars(code: str) -> dict[str, Any] | None:
    """Daily bars only. Ignores regularMarketPrice, preMarketPrice, postMarketPrice."""
    url = yahoo_source(code)
    ...
    for ts, close in zip(timestamps, closes):
        if ts is None or close is None:
            continue
        try:
            session_day = datetime.fromtimestamp(int(ts), tz=TOKYO).date()
