#!/bin/bash
# One-command install for Mac local US close collection. Beginner-safe.
# 07:00 JST is after 16:00 ET in both daylight (05:00 JST) and standard (06:00 JST) time.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
LABEL="com.marume.collect-us"
TEMPLATE="$ROOT/scripts/macos/${LABEL}.plist.template"
RUN_SCRIPT="$ROOT/scripts/macos/run-collect-us.sh"
...
chmod +x "$RUN_SCRIPT" "$ROOT/scripts/macos/install-us-schedule.sh" "$ROOT/scripts/macos/uninstall-us-schedule.sh"
...
echo "米国株の自動収集を登録しました。"
echo "毎日 07:00（Macのタイムゾーンを日本時間にしてください）に実行します。"
echo "スリープ中に時刻を過ぎた場合は、起動後に実行されます。"
echo "解除: ${ROOT}/scripts/macos/uninstall-us-schedule.sh"
