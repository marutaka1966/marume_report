#!/bin/bash
# One-command install for Mac local JP close collection. Beginner-safe.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
LABEL="com.marume.collect-jp"
TEMPLATE="$ROOT/scripts/macos/${LABEL}.plist.template"
RUN_SCRIPT="$ROOT/scripts/macos/run-collect-jp.sh"
...
chmod +x "$RUN_SCRIPT" "$ROOT/scripts/macos/install-jp-schedule.sh" "$ROOT/scripts/macos/uninstall-jp-schedule.sh"
...
echo "国内株の自動収集を登録しました。"
echo "毎日 16:10（Macのタイムゾーンを日本時間にしてください）に実行します。"
echo "スリープ中に時刻を過ぎた場合は、起動後に実行されます。"
echo "解除: ${ROOT}/scripts/macos/uninstall-jp-schedule.sh"
